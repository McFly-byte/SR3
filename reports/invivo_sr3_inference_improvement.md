...
- 梯度比：2.08 → 1.47，散斑贡献的假梯度显著降低（但仍高于 1.0，说明输出仍有比 bicubic 更强的高频成分）
...
| | 基线 | 改进后 |
|---|---|---|
| SR 图 | 盐胡椒散斑覆盖全图，中心热区模糊 | 散斑显著降低，主体椭圆结构可辨，但仍有颗粒感和外围弱信号 |
| 前向残差 | 中心蓝色（低估）+ 周边红色（高估），幅度大 | 前向误差整体下降，中心低估缓解，但残差未归零，周边仍有系统性偏差 |
| SR-bicubic 差值 | 全图随机噪点 | 结构内增强（红）、结构外去模糊（蓝），差值图不再是纯随机噪声 |
...
4/4 只大鼠、4/4 代谢物均观察到一致改善趋势。

### 6.3 仍存在的问题（有数字证据）

改进不是"完全修复"，以下问题仍然存在：

1. **背景泄漏（bg_leakage）仍显著**：改进后 SR 的背景/脑内信号比，Glx 平均 ~0.44、Lac ~0.61、Lipid ~0.60（320 样本平均）。低 SNR 代谢物（Lac、Lipid）的外围伪信号尤其明显，说明 DC 和多种子平均未能完全抑制脑区外的假信号。
2. **低频伪结构**：部分样本（如 R003/R004 的脑区下方）出现横带状弱信号，可能来自 bicubic 输入的零填充边界经 DC 放大，而非真实代谢物分布。
3. **梯度比仍 >1**：改进后梯度比 1.47（基线 2.08），说明 SR 仍比 bicubic 有更强的高频成分，其中一部分可能是真实结构增强，另一部分可能是残留的模型伪影。
4. **前向 rel-L1 未归零**：0.241 仍为非零误差，且不同代谢物差异大（Glc 0.123 vs Lac 0.308），低 SNR 代谢物改善幅度较小。

### 6.4 视觉图的局限

- comparison 图是**相对幅值**显示（每个代谢物按自身 99 百分位归一化），不同代谢物、不同样本之间的色标不可直接比大小。
- 前向残差用的是**简化 Hamming 窗算子**，不是真实 Bruker 采集，残差图只能反映近似一致性。
- 六联图只展示了一个切片一个代谢物，不能代表全部 320 样本的分布；数字汇总（6.1 节）更可靠。
- SR 面板标题已标注实际推理方法（如 "4-seed avg + DC"），旧版基线图标注为 "seed0"。
